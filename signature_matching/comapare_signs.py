...
class Signature:
    name: str
    path: str
    role: str  # "reference" or "questioned"


def run(signatures: list[Signature]):
    ...
    for s in signatures:
        ...
        feats[s.name] = {"pre": pre, "cls": cls, "patches": patches,
                         "mask": ink_patch_mask(pre["ink"], grid), "role": s.role}
        Image.fromarray(pre["display"]).save(
            os.path.join(OUT_DIR, f"preprocessed_{s.name}.png"))

    rows = []
    for a, b in itertools.combinations([s.name for s in signatures], 2):
        ...
        visualize_matches(fa["pre"]["display"], fb["pre"]["display"],
                          m["matches"], grid,
                          os.path.join(OUT_DIR, f"matches_{a}_vs_{b}.png"))
        ...
        rows.append({"pair": f"{a} vs {b}", "type": pair_type,
                     "global_cosine": g, "patch_match_score": m["score"],
                     "patch_mean_sim": m["mean_sim"], "ssim_aligned": s_ssim})

    # Step 4: baseline = mean scores over reference-reference pairs
    ref_rows = [r for r in rows if r["type"] == "ref-ref"]
    baseline = None
    if ref_rows:
        baseline = {k: float(np.mean([r[k] for r in ref_rows]))
                    for k in ("global_cosine", "patch_match_score", "ssim_aligned")}
        for r in rows:
            for k, bval in baseline.items():
                r[f"{k}_vs_baseline"] = r[k] / bval if bval > 1e-6 else float("nan")
    ...
    print(f"\n{'PAIR':<28}{'TYPE':<22}{'COSINE':>8}{'PATCH':>8}{'SSIM':>8}")
    print("-" * 74)
    for r in sorted(rows, key=lambda r: r["type"]):
        print(f"{r['pair']:<28}{r['type']:<22}"
              f"{r['global_cosine']:>8.3f}{r['patch_match_score']:>8.3f}"
              f"{r['ssim_aligned']:>8.3f}")
    if baseline:
        print("\nBaseline (mean of reference-reference pairs):")
        for k, v in baseline.items():
            print(f"  {k}: {v:.3f}")
        print("\nRatios vs baseline are in the CSV. Ratios well below 1.0 on all")
        print("three scorers = inconsistent with the reference signatures.")
    else:
        print("\nNo reference-reference pair available: interpret scores as a")
        print("clustering signal only (which signatures group together).")
    print(f"\nReport: {csv_path}")
    print(f"Visuals: {OUT_DIR}/matches_*.png, {OUT_DIR}/preprocessed_*.png")


if __name__ == "__main__":
    # Shafiul Islam signs both Letters of Authority (CMM and HCD) -> same
    # signer across two documents. Shafqat Ahmed only appears on the HCD
    # letter -> included as a different-signer control, not for baseline calc.
    SIGNATURES = [
        Signature("shafiul_cmm", "signatures/shafiul_cmm.png", "reference"),
        Signature("shafiul_hcd", "signatures/shafiul_hcd.png", "questioned"),
        Signature("shafqat_hcd", "signatures/shafqat_hcd.png", "questioned"),
    ]
    run(SIGNATURES)